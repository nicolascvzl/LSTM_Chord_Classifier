...
class ModelManager:
    ...
    def __init__(self, model: ChordModel, dataset, loss_name: str='BCE', optimizer_name: str='adam', lr=1e-3):
        self.model = model
        if CUDA_ENABLED:
        ...
        with open('data/chord_representation.pkl', 'rb') as f:
            self.chord_look_up_table = pickle.load(f)
        self.note_look_up_table = create_note_dictionary()
    ...
    def loss_and_acc_on_epoch(self, batch_size, batches_per_epoch, data_loader, train=True):
        ...
        for input_melody, chord_target in tqdm(islice(data_loader, batches_per_epoch)):
            ...
            #TODO: repackage hidden can be removed and replace with a simple init_hidden because the context is irrelevant here (unlike chunks of a same text)
        ...
        return mean_loss / batches_per_epoch, mean_notes_accuracy / batches_per_epoch, n_correct_chords / batches_per_epoch

    def mean_loss(self, chord_target, chord_output):
        assert chord_output.size() == chord_target.size()
        return self.loss(chord_target, chord_output)

    def accuracy(self, output_chord, chord_target):
        f = nn.Sigmoid()
        decoded_chord = wrap_cuda((f(output_chord) >= 0.5).type('torch.FloatTensor'))

        correct_chords = 0
        for i in range(chord_target.size()[0]):
            if (chord_target[i, :] == decoded_chord[i, :]).sum().item() == chord_target.size()[1]:
                correct_chords += 1
        correct_chords /= chord_target.size()[0]

        notes_accuracy = (chord_target == decoded_chord).sum().item() / (chord_target.size()[0] * chord_target.size()[1])
        return notes_accuracy * 100, correct_chords * 100
    ...
    def prepare_data(self, batch_size, test_batch_size, **kwargs):
        ...
        # TODO Keep padding step here, but remove the conversion!
        ...
        def collate_batch(batch):
            ...
            target = chord_to_batch(target, self.chord_look_up_table)
    ...
    def train_model(self, batch_size, batches_per_epoch, num_epochs, plot=False, save_every=2):
        ...
        for epoch_index in range(num_epochs):
            self.model.train()
            mean_loss, notes_accuracy, chord_accuracy = self.loss_and_acc_on_epoch(batch_size=batch_size, batches_per_epoch=batches_per_epoch, data_loader=train_loader, train=True)

            self.model.eval()
            val_mean_loss, val_notes_accuracy, val_chord_accuracy = self.loss_and_acc_on_epoch(batch_size=batch_size//2, batches_per_epoch=batches_per_epoch, data_loader=validation_loader, train=False)

            print(
                f'Train Epoch: {epoch_index}/{num_epochs} \t'
                f'Loss: {mean_loss}\t'
                f'Notes in chord Accuracy: {notes_accuracy} %\t'
            ...
            print(
                ...
                f'Validation Notes in chord Accuracy: {val_notes_accuracy} %\t'
        ...
        self.model.eval()
        test_mean_loss, test_notes_accuracy, test_chord_accuracy = self.loss_and_acc_on_epoch(batch_size=batch_size//2, batches_per_epoch=batches_per_epoch, data_loader=test_loader, train=False)
        print('*' * 100)
        print(
    ...
    def solve_melodies(self):
        ...
        for melody in melodies:
            ...
            predicted_chord = sigm(output) >= 0.5
